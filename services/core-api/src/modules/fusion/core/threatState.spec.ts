/**
 * CERTIFIED TEST FILE — CARRIED OVER VERBATIM.
 *
 * Source: docs/execution/certification/artifacts/cert-s/threatState.test.ts.
 * Renamed .test.ts -> .spec.ts only (this service's tsconfig excludes
 * `**\/*.spec.ts` from the build, and every other core-api test uses that
 * suffix). Not one assertion has been added, removed or relaxed: it is the
 * regression net proving the port in threatState.ts kept CERT-S semantics.
 * New behaviour introduced by WP-05 is tested in sibling spec files, never
 * by editing this one.
 */
