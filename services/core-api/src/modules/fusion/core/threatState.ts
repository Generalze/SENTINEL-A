 ...
 *
 * PORTED FROM THE CERTIFIED CERT-S ARTEFACT
 * -----------------------------------------
 * Source: docs/execution/certification/artifacts/cert-s/threatState.ts
 * (the approved starting point named by directive WP-05). The state machine
 * is reproduced here with its semantics unchanged. The complete list of
 * differences from the certified file is:
 *
 *   1. `ThreatState`, `THREAT_STATE_NAMES`, `PotentialImpact` and
 *      `OperationalSeverity` are re-exported from `@sentinel/contracts`
 *      instead of being re-declared locally. The declarations are
 *      value-for-value identical to the certified ones (0..5; LOW/MODERATE/
 *      HIGH/EXTREME; SEV1..SEV5; the same state names), so this is an import
 *      adjustment that removes a drift risk, not a behaviour change.
 *      `SourceType` and `SourceTrust` stay local because the contracts
 *      package only expresses them inline inside `NormalisedEventSchema` and
 *      does not export them as named types.
 *   2. `computeDiverseSupportingCount` is exported. It was a private helper;
 *      §65.3 requires `source_diversity` on the persisted hypothesis record,
 *      and re-deriving it in the persistence layer would risk drifting from
 *      the core's definition. Visibility only — the body is untouched.
 *
 * No other line of logic differs. `threatState.spec.ts` is the certified
 * test file, likewise carried over unchanged apart from the import path.
 *
 ...
 *
 * PORT NOTE: exported (was private in the certified artefact) so the
 * persistence layer can record §65.3 `source_diversity` from the single
 * canonical definition. Body unchanged.
 */
export function computeDiverseSupportingCount(signals: readonly ProcessedSignal[]): number {
  const ids = new Set(
