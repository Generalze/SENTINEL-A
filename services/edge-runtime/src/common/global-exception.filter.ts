...
/**
 * Always answers `{ error, trace_id }`. Internal details are logged, never
 * returned — an Edge is on a customer LAN, so its error bodies are the most
 * casually reachable output the system has.
 */
@Catch()
export class GlobalExceptionFilter implements ExceptionFilter {
  private readonly logger = new Logger(GlobalExceptionFilter.name);
    ...
    const message =
      exception instanceof HttpException ? extractMessage(exception.getResponse(), exception.message) : 'Internal server error';
